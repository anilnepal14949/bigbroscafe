            ...
            <div class="card-body">
              <div class="form-group">
                ...
                <select
                  ...
                  aria-describedby
                >
                  <option value="0" v-if="menus.length > 0">Select Menu</option>
                  <option value="0" v-else>
                    No menu found. Please add menu
                    first.
                  </option>
                  <option v-for="menu in menus" :value="menu.id" :key="menu.id">{{ menu.menu_name }}</option>
                </select>
                <small
                  class="text-danger"
                  v-if="
              ...
              <div class="form-group">
                <input type="checkbox" v-model="item.status" aria-describedby />
...
export default {
  ...
  methods: {
    ...
    onSubmit(e) {
      ...
      if (
        !this.itemErrors.menu_id &&
        !this.itemErrors.item_name &&
        !this.itemErrors.item_description &&
        !this.itemErrors.item_price &&
        !this.itemErrors.item_image
      ) {
        this.$router.push({
          name: "items",
          params: { menuId: this.item.menu_id },
        });
        this.$snotify.success("Menu Item Stored Successfully!", "Successful");
      }
    },
